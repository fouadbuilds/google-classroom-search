  ...
  return `
    ...
    <div id="overlay">
      <div id="palette">
        ...
        <div id="results">
          <div id="loading">Loading your classroom...</div>
        </div>
        <div id="footer">
  ...
  switch (type) {
    case "Assignment":
      return `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <rect x="2" y="1" width="10" height="12" rx="1.5" stroke="#4ade80" stroke-width="1.3"/>
        <path d="M4 5h6M4 7.5h4" stroke="#4ade80" stroke-width="1.1" stroke-linecap="round"/>
      </svg>`;
    case "Material":
      return `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <path d="M7 1L12 4v6L7 13 2 10V4L7 1z" stroke="#60a5fa" stroke-width="1.3"/>
      </svg>`;
    case "Announcement":
      return `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <path d="M2 5h7l2-3v8l-2-3H2V5z" stroke="#c084fc" stroke-width="1.3" stroke-linejoin="round"/>
      </svg>`;
    case "Class":
      return `<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        <rect x="1.5" y="2.5" width="11" height="9" rx="1.5" stroke="#fbbf24" stroke-width="1.3"/>
        <path d="M1.5 5.5h11" stroke="#fbbf24" stroke-width="1.1"/>
      </svg>`;
  }
}
...
async function renderResults(query: string): Promise<void> {
  ...
  if (!query) {
    ...
    if (!current.length) {
      resultsEl.innerHTML = `<div id="empty">No results for "${query}"</div>`;
...
function openPalette(): void {
  ...
  input.addEventListener("input", (e) => {
    fuseInstance = null; // reset so new data is picked up if index refreshed
